...
import {TaskDTO, TaskDTOListForDay, createTaskDTOListKey, TaskStatus} from "../../logic/domain/TaskDTO.ts";
import {
    LOAD_TASKS_REQUEST,
    LOAD_TASKS_RESPONSE,
...
export function markTaskDoneOrUndone(challengeId: number, taskProgress:TaskProgressDTO) {
    return function (dispatch, getState: ()=>ReduxState) {
        ...
        var task:TaskDTOListForDay = getState().tasks[key];
...
export function fetchTasksWhenNeeded(challengeId: number,  day: Date): any {
    return function (dispatch, getState: ()=>ReduxState) {
        ...
        if (getState().tasks[key]==null || getState().tasks[key].webState==WebState.NEED_REFETCH) {
...
export function updateTaskStatus(taskApproval:TaskApprovalDTO) {
    return function (dispatch, getState: ()=>ReduxState) {
        ...
        var challengeDTO:ChallengeDTO 
            = getState().challenges.visibleChallenges.filter(ch=>ch.id==getState().challenges.selectedChallengeId).pop();
        // user created task for himself, all others should accept it
        var challengeParticipantIds:Array<number>=challengeDTO.userLabels.map(ul=>ul.id);

        var jwtTokensOfApprovingUsers:Array<String>=getState().users.filter(a=>a.jwtToken!=null &&
